...
export default function Product({ cartItem }: { cartItem: CartItemExpanded }) {
      ...
      <div className="cart-item-details">
        ...
        <div className="product-quantity">
          <span>
            ...
            <input
              style={{
                display: enableUpdateQuantity ? "inline" : "none",
              }}
  ...
  async function updateQuantityInputOnChange(
    ...
    if (updateQuantityNew != "") {
      if (isNaN(updateQuantityNewInt)) {
        ...
        return;
      } else if (updateQuantityNewInt < QUANTITY_ADD_MIN_LIMIT_PER_REQUEST) {
        alert(
        ...
        alert("Maximum quantity available for this product is " + stock);
        return;
      } else if (updateQuantityNewInt > QUANTITY_ADD_MAX_LIMIT_PER_REQUEST) {
        alert(
  ...
  async function refreshCartAndPaymentSummary() {
    const isCartRefreshed = await refreshCart();
